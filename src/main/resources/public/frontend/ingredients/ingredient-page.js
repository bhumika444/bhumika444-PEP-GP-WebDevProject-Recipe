...
// }
/**
 * Ingredients page: add, list, delete ingredients
 */
const BASE_URL = "http://localhost:8081";

let ingredients = [];

let addIngredientNameInput;
let deleteIngredientNameInput;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  addBtn = document.getElementById("add-ingredient-submit-button");
  deleteBtn = document.getElementById("delete-ingredient-submit-button");
...
async function getIngredients() {
  try {
    ...
    if (!res.ok) {
      // if unauthorized, show empty list but do not crash
      ingredients = [];
      refreshIngredientList();
      return;
    }
    ...
    console.error("getIngredients failed:", e);
    ingredients = [];
    refreshIngredientList();
  }
}
...
async function addIngredient(e) {
  ...
  try {
    ...
    console.error("addIngredient failed:", e);
  }
}
...
async function deleteIngredient(e) {
  ...
  // Find ingredient by name (case-sensitive? tests type "tomato")
